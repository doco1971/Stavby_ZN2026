...
st.markdown("""
    ...
    .cat-content { display: flex; align-items: center; padding: 10px 0; min-height: 60px; }
    ...
    .metric-value { font-size: 1.3rem; font-weight: bold; color: #000000; }
    .v-line { position: absolute; right: 0; top: 10%; bottom: 10%; width: 1px; background-color: #e5e7eb; }

    /* KONTEJNER TABULKY - Fixní výška a scrollbar */
    .table-container { 
        height: 450px; 
        overflow: auto; 
    ...
    /* TABULKA - ČISTÉ LINKY BEZ POSUNU */
    .html-table { 
        width: 100%; 
        border-collapse: collapse; /* Zásadní pro zarovnání linek */
    ...
    .html-table th, .html-table td {
        border: 1px solid #000; /* Každá buňka má svůj border */
        padding: 6px 8px;
        white-space: nowrap;
        overflow: hidden;
        background-color: white;
...
def load_data():
    try:
        ...
        df = df.iloc[:, :23]
        # Převod na čísla tam, kde je to potřeba
...
if not df_raw.empty:
    ...
    with c_h1: st.markdown('<div style="font-size:1.2rem; font-weight:bold;">Evidence 2026</div>', unsafe_allow_html=True)
    ...
    # --- 4. VÝPOČTY (Logika: Kat I = PS+SNK+BO, Kat II = PS+BO+Poruch) ---
    ...
    # --- 5. METRIKY (Vzhled dle tvých screenshotů) ---
    ...
    # Definice šířek sloupců pro přesné lícování
    html += '<colgroup>'
