...
st.markdown("""
    ...
    .custom-head { font-size: 1.2rem; font-weight: bold; margin-bottom: 0.5rem; }
    .metric-box-styled {
    ...
    .table-container { height: 450px; overflow: auto; border: 2px solid #000; margin-top: 10px; }
...
if not st.session_state.logged_in:
    st.markdown("<h2 style='text-align: center;'>🔐 Evidence 2026</h2>", unsafe_allow_html=True)
...
# --- 3. NAČÍTÁNÍ DAT (Zde se vytváří st.session_state.data) ---
FILE_NAME = 'Soupis zakázek tabulka 2026_ZN.xlsx'

@st.cache_data(ttl=1)
def load_data_from_excel():
    try:
        # Načítáme od 6. řádku (index 5)
...
if 'data' not in st.session_state:
    st.session_state.data = load_data_from_excel()

# --- 4. HLEDÁNÍ A FILTRACE ---
c_h1, c_h2 = st.columns([1, 4])
with c_h1: st.markdown('<div class="custom-head">Evidence 2026</div>', unsafe_allow_html=True)
with c_h2: hledat = st.text_input("", placeholder="Hledat v zakázkách...", label_visibility="collapsed")

# Aktivní řádky (kde je číslo nebo název)
...
# --- 5. OPRAVENÉ VÝPOČTY SOUČTŮ ---
c1_dur = c1_zmes = c2_dur = c2_zmes = 0.0
for _, r in df_active.iterrows():
    # Kat I = PS + SNK + BO
    s1 = float(r[2] or 0) + float(r[3] or 0) + float(r[4] or 0)
    # Kat II = PS + BO + Poruch
    s2 = float(r[5] or 0) + float(r[6] or 0) + float(r[7] or 0)
